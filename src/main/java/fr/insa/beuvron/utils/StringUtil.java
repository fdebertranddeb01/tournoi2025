...
public class StringUtil implements java.io.Serializable {
  ...
  /**
   * @return true iff the string is a standard java identifier : ie begin whith a {@link
   *     java.lang.Character#isJavaIdentifierStart(char ch)} and continue whith {@link
  ...
  /**
   * write a symbol without " if it is a valid Java identifier, with " aroud, and with rewriting of
  ...
  /**
   * put beforeFirstLine at the begining (first line) of s, put beforeOtherLines at the begining of
   * the other lines.
   */
  ...
  /**
   * add " and + that allow to define a text as a string in a java source file. Attention : ne
   * fonctionne pas bien pour la dernière ligne du texte.
   */
  ...
  /**
   * "escape" les caractères sep et esc dans chacun des items, puis concatène les items en les
   * séparant par sep.
   *
   * <pre>
  ...
  /**
   * decode une chaîne encodé par {@link #encodeEscapedCSV(java.util.List, char, char)}. Voir
   * explication générale dans {@link #encodeEscapedCSV(java.util.List, char, char)}.
   *
   * <pre>
   ...
   */
  public static List<String> decodeEscapedCSV(String line, char sep, char esc)
      throws StringFormatException {
      ...
      if (modeEscape) {
        if (cur == sep || cur == esc) {
          ...
          throw new StringFormatException(
              "Escape error : " + esc + cur + " est illégal dans \"" + line + "\"");
        }
        modeEscape = false;
      } else {
        if (cur == esc) {
