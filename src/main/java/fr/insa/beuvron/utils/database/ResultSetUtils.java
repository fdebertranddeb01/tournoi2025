...
public class ResultSetUtils {
   ...
   *
   * <p>remarquez que la table affichée est assez basique mais que la méthode est trés générale :
   * elle peut afficher n'importe quel ResultSet, et pas seulement une liste de Personne
   *
      ...
      for (int i = 1; i <= nombreColonnes; i++) {
        res.append(
            "  <TD "
                + normalStyle
                + ">"
                + StringEscapeUtils.escapeHtml4("" + rs.getObject(i))
                + "</TD>\n");
      }
      res.append("</TR>\n");
    }
  ...
  /**
   * retourne le contenu du ResultSet dans deux tableau : .un tableau des noms de colonnes . un
   * tableau 2D d'objet correspondant au contenu du resultset
   *
   * @param rs
   * @return
   * @throws java.sql.SQLException
  ...
  /**
   * retourne le contenu d'un ResultSet comme un tableau (lignes) de tableaux (colonnes) d'objets
   *
   * @param rs
   * @return
   * @throws java.sql.SQLException
  ...
  /**
   * représente un ResultSet avec une {@code List<String>} pour ses noms de colonne et une {@code
   * List<List<Object>>} pour l'ensemble de ses données : chaque ligne du ResultSet produisant une
   * {@code List<Object>} représentant les valeurs de ses colonnes.
   */
