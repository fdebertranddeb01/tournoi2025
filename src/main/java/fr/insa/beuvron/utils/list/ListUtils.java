...
public class ListUtils {
  ...
  public static String formatList(
      List<? extends Object> list, String debut, String fin, String separateur) {
   ...
   */
  public static <E> String formatList(
      List<? extends E> list,
      String debut,
      String fin,
      String separateur,
   ...
   */
  public static <E> String enumerateList(
      List<? extends E> list,
      String beforeNum,
      int debutNumerotation,
      String betweenNumAndVal,
      String afterVal,
      ElemFormatter<? super E> formatter) {
    StringBuilder res = new StringBuilder();
    if (list.size() != 0) {
      int nbrDigit = (int) Math.floor(Math.log10(list.size())) + 1;
      String beforeOtherLines =
          StringUtil.mult(" ", beforeNum.length() + nbrDigit) + betweenNumAndVal;
      for (int i = 0; i < list.size(); i++) {
        String beforeFirstLine =
            beforeNum
                + String.format("%" + nbrDigit + "d", (i + debutNumerotation))
                + betweenNumAndVal;
        res.append(
            StringUtil.specialIndent(
                formatter.format(list.get(i)), beforeFirstLine, beforeOtherLines));
        res.append(afterVal);
      }
    }
    return res.toString();
  ...
  public static <E> Optional<E> selectOneOrCancel(
      String titre, List<E> list, ElemFormatter<? super E> formatter) {
    int rep = -1;
    while (rep < 0 || rep > list.size()) {
      System.out.println(titre);
      System.out.println(ListUtils.enumerateList(list, "  ", 1, " : ", "\n", formatter));
  ...
  public static <E> List<E> selectMultiple(
      String titre,
      List<E> selectionnes,
    ...
    while (rep != 0) {
      ...
      if (notSelected.isEmpty()) {
        ...
        System.out.println(
            ListUtils.enumerateList(
                notSelected, "  ", selected.size() + 1, " : ", "\n", formatter));
      }
  ...
  public static <E> List<E> selectMultiple(
      String titre, List<E> selectionnables, ElemFormatter<? super E> formatter) {
