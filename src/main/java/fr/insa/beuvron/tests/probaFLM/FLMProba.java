...
public class FLMProba {
  ...
  // -------------------------
  /**
   * Fit FLMC density to sample x (array of doubles). After calling fit(...), call getKnots(),
   * getY(), evaluate(x) etc.
   */
  ...
  /**
   * Integrals of basis functions on [left,right] inside knot interval [t0,t1]. Returns {I_left,
   * I_right} where integral = I_left*y_left + I_right*y_right.
   */
