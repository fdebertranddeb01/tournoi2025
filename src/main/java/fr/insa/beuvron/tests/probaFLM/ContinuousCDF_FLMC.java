...
public class ContinuousCDF_FLMC {
  ...
  public ContinuousCDF_FLMC(double[] sample) {
    if (sample.length == 0) throw new IllegalArgumentException("Sample must not be empty.");
    ...
    for (int i = 0; i < n; i++) F[i] = (i + 1) / (double) n;
  ...
  public List<double[]> getSegments() {
    ...
    for (int i = 0; i < a.length; i++) segs.add(new double[] {a[i], b[i]});
    return segs;
  }
