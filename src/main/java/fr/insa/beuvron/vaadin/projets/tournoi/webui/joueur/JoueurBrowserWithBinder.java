 ...
 */
@Route(value = "joueurs/browseV2", layout = MainLayout.class)
public class JoueurBrowserWithBinder extends SplitLayout implements OnlyAdmin {
	...
	private JoueurFormWithBinder form;
	...
	public JoueurBrowserWithBinder() {
		// left: grid
		...
		this.form = new JoueurFormWithBinder(Joueur.nouveauJoueur(), true);
	...
	private void onSave() {
		boolean ok = this.form.updateModel();
		if (ok) {
			try (Connection con = ConnectionPool.getConnection()) {
				Joueur j = this.form.getEditedJoueur();
				j.updateSaufID(con);
	...
	private void onReload() {
		try (Connection con = ConnectionPool.getConnection()) {
			...
			this.form.setEditedJoueur(reloaded);
